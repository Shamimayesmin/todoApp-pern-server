...
app.post("/todos", async (req, res) => {
	try {
		...
		console.log("Data inserted into DB:", newTodo.rows[0]);
...
app.get("/todos/:id", async (req, res) => {
	try {
		const { id } = req.params;
		const todo = await pool.query("SELECT * FROM users WHERE id=$1", [id]);
		res.status(200).json({
			message: `specific todos is returned`,
			data: todo.rows,
		});
	} catch (error) {
		res.json({
...
app.delete("/todos/:id", async (req, res) => {
	try {
		...
		res.status(200).json({
			message: `specific todo is deleted with id: ${id}`,
			data: deleteTodo.rows,
		});
	} catch (error) {
		res.json({
			error: error.message,
